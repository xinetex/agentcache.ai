...
const NODE_CATALOG = {
  ...
  // Storage
  storage_s3: { id: 'storage_s3', category: 'output', latency: 200, cost: 0.05 },
};
...
function generatePipeline(request) {
  ...
  // 6. Add storage node if requested
  if (intent.isStorage) {
    const storageId = `storage_${nodeIdCounter++}`;
    nodes.push({
      id: storageId,
      type: 'storage_s3',
      position: { x: 100 + nodeIdCounter * 180, y: 350 },
      config: {
        bucket: 'agentcache-data',
        encryption: 'AES256',
      },
    });

    // Connect from last node (usually LLM or audit)
    const lastNodeId = nodes[nodes.length - 2]?.id; // -2 because we just pushed
    if (lastNodeId) {
      connections.push({ from: lastNodeId, to: storageId });
    }
  }
...
function parseIntent(prompt) {
  ...
  return {
    ...
    isStorage: lower.includes('file') || lower.includes('save') || lower.includes('upload') || lower.includes('storage'),
  };
}
