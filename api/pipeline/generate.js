...
function applyPerformanceSettings(baseNodes, performance) {
  ...
  // Storage Optimization (Custom handling for file/storage requests)
  if (baseNodes.some(n => n.use_case === 'storage_optimization') || performance === 'storage') {
    return [
      { type: 'cache_l1', config: { ttl: 60, max_size: '100MB' } }, // Light L1
      { type: 'cache_l2', config: { ttl: 86400, storage: 'redis', persistence: true } }, // Robust L2
      { type: 'cache_l3', config: { ttl: 31536000, storage: 's3_compatible', encrypt: true } }, // Long-term L3
...
function extractUseCase(prompt) {
  ...
  // Storage/File patterns (User request handling)
  if (lower.includes('file') || lower.includes('storage') || lower.includes('save') || lower.includes('upload')) return 'storage_optimization';
