  ...
  if (strategy === 'parallel' || strategy === 'consensus') {
    ...
    const promises = models.map(async (modelConfig, idx) => {
      ...
      try {
        ...
        // Cache miss - would call actual LLM here
        // For now, return indication that external call needed
        const span = {
          spanId,
          traceId,
          provider: modelConfig.provider,
        ...
        return {
          ...
          cacheMiss: true,
          cacheKey,
          message: 'Cache miss - call your LLM provider and store via /api/cache/set',
          span
        };
      } catch (err) {
        const span = {
