...
const PORTAL_USER_CACHE_TTL_SECONDS = Number(process.env.PORTAL_USER_CACHE_TTL_SECONDS || 300);

function portalUserCacheKey(userId) {
  return `portal:user:${userId}`;
}

function normalizeCachedUser(raw) {
  if (!raw) return null;
...
export async function requireAuth(req) {
  ...
  const cachedUser = normalizeCachedUser(await redis.get(portalUserCacheKey(userId)));
  if (cachedUser) {
    return cachedUser;
  }
  ...
  try {
    ...
    const result = await client.query(`
      SELECT id, email, full_name, name, organization_id, org_id, role, is_active
      FROM users
      WHERE id = $1
      LIMIT 1
    `, [userId]);
    ...
    const resolvedUser = {
      ...
      full_name: user.full_name || user.name || decoded.email?.split('@')[0] || 'Workspace User',
      organizationId: user.organization_id || user.org_id || decoded.organizationId || decoded.orgId || null,
      role: user.role || decoded.role || 'member',
    };

    await redis.setex(portalUserCacheKey(userId), PORTAL_USER_CACHE_TTL_SECONDS, JSON.stringify(resolvedUser));
    return resolvedUser;
  } finally {
    await client.end();
  }
...
/**
 * Resolve thrown auth errors to HTTP status codes for Node-style API routes.
 * @param {Error} error
 * @returns {number|null}
 */
