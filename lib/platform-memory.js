...
// Decay Configuration (IP Protection: Loaded from env)
const DECAY_RATE = parseFloat(process.env.MEMORY_DECAY_RATE || '0.05');
const REINFORCE_RATE = parseFloat(process.env.MEMORY_REINFORCE_RATE || '0.1');
const VITALITY_THRESHOLD = parseFloat(process.env.MEMORY_VITALITY_THRESHOLD || '0.3');
...
class PlatformMemory {
  ...
  async get(namespace, key, options = {}) {
    ...
    try {
      const results = await sql`
        SELECT data, confidence, vitality, created_at, hit_count
      ...
      if (results.length > 0) {
        ...
        // Reinforce Vitality (Biological Memory)
        await this.reinforce(namespace, key, cached.vitality);
      ...
      if (options.semantic_search) {
        const vectorQuery = {
          ...
          filter: options.region ? `region = '${options.region}'` : undefined
        };

        let vectorResults = await vectorIndex.query(vectorQuery);

        // Elasticity: Failover to Global if Local misses
        if (vectorResults.length === 0 && options.region) {
          // console.log('Elasticity: Local miss, expanding to Global Mesh...');
        ...
        if (vectorResults.length > 0 && vectorResults[0].score >= (options.min_similarity || 0.8)) {
          const match = vectorResults[0];
          return {
            hit: true,
            tier: 'L3',
        ...
        // Swarm Trigger: If we still miss, we could trigger a GOAP swarm here
        // if (options.enable_swarm) { ... }
      }
  ...
  async set(namespace, key, data, options = {}) {
    const {
      ...
      reasoning = null,
      metadata = {},
      region = process.env.AWS_REGION || process.env.VERCEL_REGION || 'global'
    } = options;
    ...
    try {
      ...
      // Store in L3 (Vector) if meaningful text
      if (reasoning || typeof data === 'string' || data.description) {
        const textData = reasoning || (typeof data === 'string' ? data : JSON.stringify(data));
  ...
  async prune(namespace, options = {}) {
    ...
    try {
      const maxAgeDate = new Date(Date.now() - max_age_days * 24 * 60 * 60 * 1000);

      const result = await sql`
        DELETE FROM platform_memory_cache
        WHERE namespace = ${namespace}
          AND (
            (created_at < ${maxAgeDate} AND hit_count < ${min_hit_count})
            OR expires_at < NOW()
            OR (vitality < ${options.min_vitality || 0} AND vitality IS NOT NULL)
          )
      `;
  ...
  /**
   * Reinforce memory vitality on access
   */
  async reinforce(namespace, key, currentVitality) {
    const newVitality = Math.min(1.0, parseFloat(currentVitality || 1.0) + REINFORCE_RATE);
    try {
      await sql`
        UPDATE platform_memory_cache
  ...
  /**
   * Apply temporal decay to all memories in namespace
   * "Forgetting" unused information
   */
  async decay(namespace) {
    try {
      // Reduce vitality for items not accessed recently
      const result = await sql`
        UPDATE platform_memory_cache
        SET vitality = vitality - ${DECAY_RATE},
            last_decay_at = NOW()
        WHERE namespace = ${namespace}
      ...
      // Prune dead memories
      await this.prune(namespace, { min_vitality: VITALITY_THRESHOLD });
