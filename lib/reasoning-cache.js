...
//
// Pure and I/O-free (no node:crypto — edge-safe) so every rule is unit-tested. The handler
// (api/cache/reasoning.ts) is the thin Redis shell around this.

// Pure, synchronous, runtime-agnostic 64-bit fingerprint -> 16 lowercase hex
// chars. NOT cryptographic (it is a cache key, not a secret) — implemented
// without node:crypto so this module works identically in Node AND in Edge/Web
// runtimes (Vercel Edge functions cannot import node:crypto).
function hash16(str) {
...
export function taskFingerprint(task) {
  return hash16(normalizeTask(task));
}
