...
async function call(path, method, body, apiKey, extra = {}) {
    const headers = {
...
export const CacheTools = {
    ...
    handlers: {
        agentcache_get: async (args, ctx) => {
            const ns = args?.namespace || 'default';
            const extra = { 'X-Cache-Namespace': ns };
            if (args?.model)
                extra['X-Model'] = String(args.model);
            if (args?.inputTokens != null)
                extra['X-Input-Tokens'] = String(args.inputTokens);
            if (args?.outputTokens != null)
                extra['X-Output-Tokens'] = String(args.outputTokens);
            const data = await call(`/api/cache/get?key=${encodeURIComponent(args?.key || '')}`, 'GET', null, ctx.apiKey, extra);
            return ok(data);
        },
        agentcache_set: async (args, ctx) => {
            const ns = args?.namespace || 'default';
