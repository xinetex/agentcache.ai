(function () {
  ...
  async function fetchJson(url, options) {
    ...
    if (!response.ok) {
      ...
      error.upgradeUrl = payload && payload.upgradeUrl ? payload.upgradeUrl : null;
      throw error;
    }
  ...
  function renderBillingCard() {
    ...
    const recommendation = billing.recommendations && billing.recommendations.upgradeTarget
      ? billing.recommendations.upgradeTarget
      : null;

    const planName = subscription.displayPlan || humanizePlan(subscription.publicPlan || subscription.plan);
    const percentUsed = Number(usage.percentUsed || 0);
    const remaining = Math.max(Number(usage.remaining || 0), 0);

    setText('billing-plan-name', `${planName} plan`);
    setText(
      'billing-plan-copy',
      recommendation
        ? `${recommendation.name} is the next workspace upgrade when you need more throughput or limits.`
        : 'This workspace is already on the highest self-serve tier.'
    );
    setText(
      'billing-usage-copy',
      `${formatCompactNumber(usage.requestsThisMonth || 0)} / ${formatCompactNumber(usage.quota || 0)} requests used this month. ${formatCompactNumber(remaining)} remaining.`
    );
    setText(
      'billing-namespace-usage',
      `${resources.namespaces?.used || 0} / ${resources.namespaces?.limit || 0}`
    );
    setText(
      'billing-api-key-usage',
      `${resources.apiKeys?.used || 0} / ${resources.apiKeys?.limit || 0}`
    );

    const usageBar = document.getElementById('billing-usage-bar');
    if (usageBar) {
      usageBar.style.width = `${Math.max(percentUsed, percentUsed > 0 ? 8 : 0)}%`;
    }

    const upgradeLink = document.getElementById('billing-upgrade-link');
    if (upgradeLink) {
      if (recommendation) {
        upgradeLink.href = recommendation.upgradeUrl;
        upgradeLink.textContent = `Upgrade to ${recommendation.name}`;
      } else if (subscription.stripeSubscriptionId) {
        upgradeLink.href = '/upgrade.html?plan=enterprise';
        upgradeLink.textContent = 'Manage billing';
      } else {
        upgradeLink.href = '/pricing.html';
        upgradeLink.textContent = 'View plans';
      }
    }
  }
  ...
  function renderAddons() {
    ...
    const addons = state.billing && Array.isArray(state.billing.addons)
      ? state.billing.addons
      : [];

    if (addons.length === 0) {
      container.innerHTML = `
        <div class="rounded-2xl border border-slate-800 bg-slate-900/60 px-4 py-3 text-sm text-slate-400">
          No add-on data is available for this workspace yet.
        </div>
      `;
      return;
    }
    ...
    container.innerHTML = addons.map((addon) => {
      const statusLabel = addon.included
        ? 'Included'
        : addon.active
          ? 'Active'
          : addon.upgradeRequired
            ? 'Upgrade plan'
            : 'Available';
      const statusTone = addon.included
          ...
          <div class="flex items-start justify-between gap-3">
            <div>
              <div class="flex items-center gap-2">
                <div class="text-sm font-medium text-slate-100">${escapeHtml(addon.name)}</div>
                <span class="rounded-full border px-2 py-0.5 text-[0.68rem] font-medium ${statusTone}">${statusLabel}</span>
              </div>
              <div class="mt-1 text-sm text-slate-400">${escapeHtml(addon.description)}</div>
              <div class="mt-2 text-xs text-slate-500">${escapeHtml(addon.usageSummary || '')}</div>
            </div>
            <a
              href="${escapeHtml(addon.upgradeUrl || addon.checkoutUrl || '/pricing.html')}"
              class="inline-flex shrink-0 items-center rounded-xl border border-slate-700 bg-slate-950/70 px-3 py-2 text-sm font-medium text-slate-100 transition hover:bg-slate-950"
            >
              ${ctaLabel}
            </a>
          </div>
  ...
  async function refreshDashboard() {
    ...
    const [meResult, dashboardResult, billingResult, pipelineResult] = await Promise.allSettled([
    ...
    if (billingResult.status === 'fulfilled' && billingResult.value) {
      ...
      showAlert(`Could not load billing state: ${billingResult.reason.message}`, 'warning');
    }
    ...
    if (billingState === 'success') {
      showAlert('Workspace billing updated. Stripe may take a few moments to sync refreshed quotas and limits.', 'success');
    } else if (billingState === 'cancel') {
      showAlert('Billing update canceled. Your workspace plan has not changed.', 'warning');
    }
    ...
    if (billingState === 'success' || billingState === 'cancel') {
      // Preserve the billing banner over generic onboarding nudges.
    } else if (!hasPrimaryLoadError && apiKeys.length === 0) {
  ...
  async function generateApiKey() {
    ...
    try {
      ...
      if (error.upgradeUrl) {
        showAlert(`API key limit reached. Redirecting to ${error.payload?.recommendedPlan || 'the next plan'} upgrade...`, 'warning');
        window.setTimeout(() => {
